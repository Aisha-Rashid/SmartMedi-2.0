...
if(isset($_POST['kidData'])!=""){
	...
	$num_boxes = mysqli_real_escape_string($db, $_POST['num_boxes']); 
	
	for ($i = 1; $i <= $num_boxes; $i++) {
    if (isset($_POST['box_' . $i])) {
      //$box_value = $_POST['box_' . $i];
	  ...
	  $spousetel = mysqli_real_escape_string($db, $_POST['spousetel']);
	  
      $sql = "INSERT INTO dependants (IDNo, spousename, spousetel, kidName, kidDOB, kidGender, kidBlood, kidAllergies, kidConditions) VALUES ('$unique', '$spousename', '$spousetel', '$box_name', 
	  '$box_DOB', '$box_gender', '$box_allergies', '$box_conditions')";
